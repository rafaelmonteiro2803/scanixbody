...
export const Card = React.forwardRef<HTMLDivElement, CardProps>(
  ({ variant = 'default', noPadding = false, accent = false, hoverable = false, children, className, ...props }, ref) => (
    <div
      ref={ref}
      className={cn(
    ...
    <div ref={ref} className={cn('flex items-start justify-between gap-3 mb-4', className)} {...props}>
      {(title || subtitle) ? (
        <div className="min-w-0 flex-1">
          {title && <h3 className="text-base font-semibold text-text-primary leading-tight truncate">{title}</h3>}
          {subtitle && <p className="mt-0.5 text-xs text-text-secondary leading-relaxed">{subtitle}</p>}
        </div>
      ) : (
...
export const CardBody = React.forwardRef<HTMLDivElement, CardBodyProps>(
  ({ children, className, ...props }, ref) => (
    <div ref={ref} className={cn('flex-1', className)} {...props}>{children}</div>
  ),
);
...
export const CardFooter = React.forwardRef<HTMLDivElement, CardFooterProps>(
  ({ divider = false, children, className, ...props }, ref) => (
    <div
      ref={ref}
      className={cn('mt-4 flex items-center gap-3', divider && 'pt-4 border-t border-border', className)}
      {...props}
    >
      {children}
    </div>
