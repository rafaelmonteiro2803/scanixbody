 ...
 * client (`createAdminClient`) so they are safe to call only from trusted
 * server-side code.
 */
...
const adminService = {
   ...
   * Supports filtering by role, status, and a case-insensitive email search.
   */
  async getUsers(filters?: UserFilters): Promise<UserWithProfile[]> {
    ...
    // Build the profiles query (profiles drive the join).
    ...
    // Fetch matching auth users via Admin API.
    const { data: authUsersData, error: authError } =
      await adminSupabase.auth.admin.listUsers({
        page: 1,
        perPage: 1000, // fetch enough to cover the profile slice
      })
    ...
    let results: UserWithProfile[] = (profiles as ProfilesRow[])
      ...
      .filter((u: UserWithProfile | null): u is UserWithProfile => u !== null)

    // Apply email search after joining (Supabase Auth Admin API has no
    // server-side ilike support in the JS SDK).
    if (filters?.search) {
   ...
   * confirmation).  A profile row is automatically created by a Supabase
   * trigger; if that trigger is not in place this method also upserts one.
   *
   * @returns The newly created Supabase Auth user record.
   */
  async createUser(data: CreateUserDTO): Promise<UserWithProfile> {
    ...
    // Ensure a profile row exists (in case the DB trigger hasn't fired yet).
    ...
    void auditService.log(
      ...
      newUser.id,
      { email: newUser.email, role: data.role ?? 'usuario_final' },
    )
  ...
  async resetUserPassword(userId: string): Promise<string> {
    ...
    if (error) {
      throw new AdminServiceError(
        `resetUserPassword failed: ${error.message}`,
      )
    }

    // Mark the account for forced password change on next login.
    await adminSupabase
      .from('profiles')
      .update({
        status: 'first_access',
  ...
  /**
   * Blocks a user account: sets `status = 'blocked'` in the profile and
  ...
  async blockUser(userId: string): Promise<void> {
    ...
    if (signOutError) {
      // Log but don't throw – the profile is already blocked which is the
      // critical operation.
      console.error(
        '[AdminService] blockUser – could not revoke sessions:',
  ...
  async unblockUser(userId: string): Promise<void> {
    ...
    if (error) {
      throw new AdminServiceError(
        `unblockUser failed: ${error.message}`,
        error.code,
      )
    }
  ...
  async getAuditLogs(filters?: AuditLogFilters): Promise<AuditLog[]> {
    ...
    if (error) {
      throw new AdminServiceError(
        `getAuditLogs failed: ${error.message}`,
        error.code,
      )
    }
  ...
  /**
   * Returns total user counts grouped by status (for admin dashboard cards).
   */
  async getUserStats(): Promise<Record<string, number>> {
    ...
    if (error) {
      throw new AdminServiceError(
        `getUserStats failed: ${error.message}`,
        error.code,
      )
    }
