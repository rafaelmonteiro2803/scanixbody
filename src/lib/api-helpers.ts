 ...
 *
 * Returns `{ data, error }`.  When parsing fails the error message is human-
 * readable; `data` is `null` in that case.
 */
export async function parseBody<T = unknown>(
  ...
  try {
    ...
    if (!text || text.trim() === '') {
      return { data: null, error: 'Corpo da requisição está vazio' }
    }
    const parsed = JSON.parse(text) as T
    return { data: parsed, error: null }
  } catch {
    return { data: null, error: 'Corpo da requisição não é um JSON válido' }
  }
}
 ...
 *
 * Returns `{ data, error }`.  `error` contains the Zod flatten structure so
 * route handlers can forward field-level errors to the client.
 */
...
/**
 * Retrieves the authenticated Supabase user and their profile role from the
...
async function getAuthContext(
  ...
  try {
    ...
    if (userError || !user) {
      return { context: null, error: 'Não autenticado' }
    }
    ...
    if (profileError || !profile) {
      return { context: null, error: 'Perfil não encontrado' }
    }
    ...
    return { context: null, error: 'Erro interno de autenticação' }
  }
}
...
export function withAuth(handler: AuthedHandler) {
  return async (
    ...
    if (!context) {
      return createErrorResponse(error ?? 'Não autenticado', 401)
    }
